...
const ParticleBackground = () => {
  ...
  const renderLightTheme = () => {
    const sx = 120;
    const sy = 80;
    // center angles for rays (deg)
    const centers = [20, 60, 100, 140, 180, 220, 260, 300];
    const halfWidth = 17; // half of 34 degrees
    const radii = [900, 820, 860, 900, 940, 880, 820, 780];

    const makePoint = (angleDeg, r) => {
      const rad = (angleDeg * Math.PI) / 180;
      const x = Math.round(sx + r * Math.cos(rad));
      const y = Math.round(sy + r * Math.sin(rad));
      return `${x},${y}`;
    };

    const gradients = ['fadeRay1', 'fadeRay2', 'fadeRay3', 'fadeRay4'];

    const rays = centers.map((c, i) => {
      const left = makePoint(c - halfWidth, radii[i]);
      const right = makePoint(c + halfWidth, radii[i]);
      const gid = gradients[i % gradients.length];
      const delay = `${(i % 3) * 300}ms`;
      const d = `M ${sx},${sy} L ${left} L ${right} Z`;
      return (
        <path key={i} className="ray" d={d} fill={`url(#${gid})`} opacity={0.78} style={{ animationDelay: delay }} />
      );
    });
  ...
  return (
    <div ref={containerRef} className={`particle-background ${theme}`}>
      {theme === 'light' ? renderLightTheme() : renderNightTheme()}
    </div>
  );
};
